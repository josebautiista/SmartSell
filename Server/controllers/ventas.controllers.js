      ...
      if (error) {
        ...
        connection.query(updateVentaQuery, [venta_id, venta_id], (error) => {
          if (error) {
            ...
            detalles.forEach((detalle) => {
              actualizarCantidadesProductos(detalle);
            });
...
function actualizarCantidadesProductos(detalle) {
  ...
  connection.query(
    restarCantidadQuery,
    [detalle.cantidad, productoId],
    (err) => {
      if (err) {
        console.error("Error al restar la cantidad del producto:", err);
      }
    }
  );
}
