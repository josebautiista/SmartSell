...
exports.getProductosEnCarrito = (req, res) => {
  ...
  // Realiza una consulta a la base de datos para obtener los productos del pedido asociados a la mesa
  const query = `
    SELECT cc.*, p.* 
    FROM pedido cc
  ...
  connection.query(query, [mesaId], (error, results) => {
    if (error) {
      console.error("Error al obtener productos en pedido:", error);
      res.status(500).json({
        error: "Error al obtener productos en pedido",
      });
    } else {
...
exports.getProductoExiste = (req, res) => {
  ...
  connection.query(query, [mesaId, productoId], (error, results) => {
    if (error) {
      console.error("Error al obtener el producto en pedido:", error);
      res.status(500).json({
        error: "Error al obtener el producto en pedido",
      });
    } else {
